...
# ▼▼▼ 追加：問題番号リストファイルから検索（txt/csv対応） ▼▼▼
list_file = st.file_uploader(
    "問題番号リストをドラッグ＆ドロップ（例: 99D82, 98D85... / txt・csv可）",
    type=["txt", "csv"]
)


def extract_question_ids(uploaded_file) -> list[str]:
    """アップロードされたtxt/csvから 99D82 / 103C40 / 99D84- 形式を抽出"""
    ...
    ids = re.findall(r"\b\d{2,3}[A-Da-d]\d{1,3}-?\b", text)

    # 大文字化・末尾ハイフン除去・重複除去（順序保持）
    out, seen = [], set()
    for qid in ids:
...
def filter_by_question_ids(df: pd.DataFrame, qids: list[str]) -> tuple[pd.DataFrame, list[str]]:
    """問題番号リストの順番でDBから抽出。見つからない番号も返す。"""
    if not qids:
        return pd.DataFrame(columns=df.columns), []

    text_cache = df.apply(lambda row: row_text(row).upper(), axis=1)
    rows = []
    missing = []

    for qid in qids:
        # 例: 99D82 が [99D82-ori] や 99D82- に含まれていても拾う
        pattern = rf"(?<![0-9A-Z]){re.escape(qid)}(?:-|\b)"
        hit_idx = text_cache[text_cache.str.contains(pattern, regex=True, na=False)].index
        if len(hit_idx) == 0:
            missing.append(qid)
        else:
            rows.append(df.loc[hit_idx[0]])  # 同じ番号が複数あれば最初の1件
...
uploaded_qids = extract_question_ids(list_file)

if uploaded_qids:
    df_filtered, missing_qids = filter_by_question_ids(df, uploaded_qids)
    st.success(f"問題番号リストから {len(df_filtered)} 件ヒットしました（入力 {len(uploaded_qids)} 件）")
    if missing_qids:
        st.warning("DB内で見つからなかった問題番号: " + ", ".join(missing_qids))
    file_prefix = f"問題番号リスト検索_{datetime.now().strftime('%Y%m%d%H%M%S')}"
else:
    if not query:
        st.stop()
    ...
    file_prefix = f"{(query if query else '検索なし')}{datetime.now().strftime('%Y%m%d%H%M%S')}"
# ▲▲▲ 追加ここまで ▲▲▲
